...
BIBFILES=template/master.bib template/dedup.bib \
	template/meta-heuristics.bib $(MYBIBFILES)
...
STYLES=	template/acmtrans2m.cls template/acmtrans.bst \
	template/acmsmall.cls template/ACM-Reference-Format-Journals.bst \
...
# Ensure that we replace the old PDF with a new one in one 'mv' command
# so PDF viewers don't crash trying to re-read a PDF being overwritten.
%.pdf: %.ps
	ps2pdf14 -dEPSCrop $? tmp.$@ || ps2pdf -dEPSCrop $? tmp.$@
	mv tmp.$@ $@
# the old rule for ps2pdf is too old.
...
clean: FRC
	...
	$(RM) $(RMFLAGS) $(FIGS_EPS) $(FIGS_PDF) anonmap.txt ${EXTRACLEAN}
	$(RM) $(RMFLAGS) tags
